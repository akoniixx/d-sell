...
export const TablePagination: React.FC<TablePaginationProp> = ({
  ...
  return (
    <div className='py-5 text-center'>
      <a onClick={clickLeftArrow} className='btn btn-icon btn-sm btn-light-primary mr-2 my-1'>
        <i className='ki ki-bold-double-arrow-back icon-xs'></i>
      </a>
      {_.range(0, Math.ceil(allPage), 1).map((p: number) => (
        <a
          key={p}
          onClick={() => setCurrentPage(() => p * limitRowPerPage)}
          className={
            currentPage === p * limitRowPerPage
      ...
      <a onClick={clickRightArrow} className='btn btn-icon btn-sm btn-light-primary mr-2 my-1'>
